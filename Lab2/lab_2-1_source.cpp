...
int main(int argc, const char *argv[])
{
    ...
    image =cv::imread("bonus.png",-1);
    
    vector<cv::Mat> split_bgr;
    cv::split(image,split_bgr);
    //split_bgr.push_back(cv::Mat(image_size, CV_8UC1));
    cv::Mat framebuffer_compat;

    split_bgr.pop_back();
    cv::merge(split_bgr , framebuffer_compat);
    cv::Mat res;
    cv::cvtColor(framebuffer_compat,res,cv::COLOR_BGR2BGR565);
    //cv::Mat image_bmp;
...
// get image size of the image.
    ...
    image_size = framebuffer_compat.size();
	...
	ofs.write(reinterpret_cast<char*>(res.ptr(y)),image_size.width*2);
    }
...
struct framebuffer_info get_framebuffer_info(const char *framebuffer_device_path)
{
    ...
    // put the required attributes in variable "fb_info" you found with "ioctl() and return it."
     //fb_info.xres_virtual =    480;   // 480
    // fb_info.bits_per_pixel =  32;   // 32
